...
const gamePadEffect = (prevGamePads) => {
  ...
  return batch([
    batch(newlyConnected.map(gp => ({ type: 'GAMEPAD_CONNECTED', gamePad: gp }))),
    batch(newlyDisconnected.map(gp => ({ type: 'GAMEPAD_DISCONNECTED', gamePadIndex: gp.index }))),
    batch(changeMessages),
  ]);
};
